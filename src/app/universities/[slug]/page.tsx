...
export default async function UniversityPage({
  ...
  const [university, universityMajors] = await Promise.all([
    ...
    // We'll load majors after confirming uni exists
    Promise.resolve([]),
  ]);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#faf7f2]">
      ...
      <main className="flex-1">
        ...
        <div className="bg-gradient-to-br from-[#1a3a5c] to-[#2a5a8c] text-white py-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-start gap-5">
            <div className="w-16 h-16 rounded-2xl bg-white shadow flex items-center justify-center flex-shrink-0">
              {university.logo_url ? (
                <img src={university.logo_url} alt={university.name_en} className="w-12 h-12 object-contain" />
              ) : (
            ...
            <div>
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                <span className="bg-[#d4a843]/20 text-[#d4a843] text-xs font-semibold px-2 py-0.5 rounded-full font-cairo">
                  {typeLabels[university.type]?.ar} / {typeLabels[university.type]?.en}
                </span>
              ...
              <h1 className="text-2xl sm:text-3xl font-black font-cairo">{university.name_ar}</h1>
              <p className="text-blue-200 text-sm font-cairo">{university.name_en}</p>
              <div className="flex items-center gap-4 mt-3 flex-wrap text-sm text-blue-200 font-cairo">
                <span className="flex items-center gap-1"><MapPin size={14} />{university.location_ar}</span>
        ...
        {/* Stats bar */}
        {(university.faculties_count || university.total_students || university.tuition_min) && (
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex gap-8 flex-wrap">
              {university.faculties_count && (
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-base font-bold text-[#1a3a5c] font-cairo">{university.faculties_count}</p>
                    <p className="text-xs text-gray-400 font-cairo">كلية / Faculties</p>
                  </div>
                </div>
              )}
              {university.total_students && (
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-base font-bold text-[#1a3a5c] font-cairo">{university.total_students.toLocaleString()}</p>
                    <p className="text-xs text-gray-400 font-cairo">طالب / Students</p>
                  </div>
                </div>
              )}
              {university.tuition_min && (
                <div className="flex items-center gap-2">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 font-cairo">مصروفات تبدأ من / Tuition from</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {university.description_ar && (
                <div className="bg-white rounded-2xl p-5 border border-gray-100">
                  <h2 className="font-bold text-[#1a3a5c] font-cairo mb-3">عن الجامعة / About</h2>
                  <p className="text-sm text-gray-600 font-cairo leading-relaxed">{university.description_ar}</p>
                  {university.description_en && (
                    <p className="text-xs text-gray-400 font-cairo mt-2 leading-relaxed">{university.description_en}</p>
                  )}
                </div>
              )}
              ...
              {majors.length > 0 && (
                <div className="bg-white rounded-2xl p-5 border border-gray-100">
                  ...
                  <div className="space-y-3">
                    {majors.map((um) => (
                      <div key={um.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                        <div>
                          <p className="text-sm font-semibold text-[#1a3a5c] font-cairo">{um.major?.name_ar}</p>
                          <p className="text-xs text-gray-400 font-cairo">{um.major?.name_en}</p>
                        </div>
                        <div className="text-right">
                          {um.tuition_per_year && (
                            <p className="text-xs font-bold text-[#d4a843] font-cairo">
                              {um.tuition_per_year.toLocaleString()} {um.currency ?? "EGP"}/year
                            </p>
                          )}
                          {um.min_score && (
                            <p className="text-xs text-gray-400 font-cairo">أدنى درجة: {um.min_score}%</p>
                          )}
                        </div>
                      </div>
                    ))}
            ...
            <div className="space-y-4">
              ...
              {university.accreditations?.length ? (
                <div className="bg-white rounded-2xl p-5 border border-gray-100">
                  <h3 className="font-bold text-[#1a3a5c] font-cairo text-sm mb-3">الاعتمادات / Accreditations</h3>
                  <div className="flex flex-wrap gap-2">
                    {university.accreditations.map((a) => (
                      <span key={a} className="text-xs bg-green-50 text-green-700 px-2 py-1 rounded-full font-cairo">
                        {a}
                      </span>
                    ))}
                  </div>
              ...
              <div className="bg-[#1a3a5c] rounded-2xl p-5 text-white">
                <h3 className="font-bold font-cairo text-sm mb-2">قارن مع جامعات أخرى</h3>
                <p className="text-blue-200 text-xs font-cairo mb-3">Compare with other universities side-by-side</p>
                <a
                  href="/compare"
