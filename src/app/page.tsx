      ...
      <main className="flex-1">
          ...
          <div className="relative mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 lg:px-8 lg:grid-cols-[1.05fr,0.95fr] lg:py-24">
            <div className={`flex flex-col justify-center ${isRtl ? 'text-right' : 'text-left'}`}>
              ...
              <h1 className="mt-6 text-4xl font-black leading-tight text-[#1a3a5c] font-cairo sm:text-5xl lg:text-6xl">
                {t("hero.title")}
                <span className="block text-[#d4a843] font-playfair italic">{t("hero.titleHighlight")}</span>
              </h1>
              ...
              <div className="mt-10 grid gap-3 sm:grid-cols-3">
                {pillars.map((pillar) => (
                  <div key={pillar.key} className="rounded-3xl border border-white/70 bg-white/70 p-4 shadow-sm backdrop-blur">
                    <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-2xl bg-[#1a3a5c]/6">
                      {pillar.icon}
                    </div>
                    <h2 className="text-sm font-bold text-[#1a3a5c] font-cairo">{t(`${pillar.key}.title`)}</h2>
                    <p className="mt-2 text-xs leading-6 text-gray-500 font-cairo">{t(`${pillar.key}.text`)}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="relative">
              <div className="rounded-[32px] border border-white/70 bg-white/90 p-5 shadow-[0_30px_80px_rgba(15,36,60,0.12)] backdrop-blur">
                <div className={`rounded-[28px] bg-[#173754] p-6 text-white ${isRtl ? 'text-right' : 'text-left'}`}>
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <div className={`rounded-3xl border border-gray-100 bg-[#faf7f2] p-4 ${isRtl ? 'text-right' : 'text-left'}`}>
                    <p className="text-xs uppercase tracking-wide text-gray-400">{t("preview.factors")}</p>
                  ...
                  <div className={`rounded-3xl border border-gray-100 bg-[#fff9ee] p-4 ${isRtl ? 'text-right' : 'text-left'}`}>
                    <p className="text-xs uppercase tracking-wide text-gray-400">{isRtl ? "وضع المقارنة" : "Compare mode"}</p>
        ...
        <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
          <div className={`mb-10 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between ${isRtl ? 'text-right' : 'text-left'}`}>
            <div>
              <h2 className="text-3xl font-black text-[#1a3a5c] font-cairo">{t("features.title")}</h2>
          ...
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {features.map((feature) => (
              <Link
                key={feature.href}
                href={feature.href}
                className={`group rounded-[28px] border border-white/70 bg-white/80 p-5 shadow-sm transition-all hover:-translate-y-1 hover:border-[#d4a843]/30 hover:shadow-lg ${isRtl ? 'text-right' : 'text-left'}`}
              >
                <div className={`flex h-12 w-12 items-center justify-center rounded-2xl bg-[#faf7f2] transition-colors group-hover:bg-[#fff4d9] ${isRtl ? 'mr-0 ml-auto' : 'ml-0 mr-auto'}`}>
                  {feature.icon}
                </div>
                <h3 className="mt-5 text-lg font-bold text-[#1a3a5c] font-cairo">{t(`${feature.key}.title`)}</h3>
                <p className="mt-4 text-sm leading-7 text-gray-500 font-cairo">{t(`${feature.key}.desc`)}</p>
              </Link>
            ))}
          </div>
        </section>
        ...
        <section className="border-y border-[#d4a843]/20 bg-[#fff9ee] py-16">
          <div className="mx-auto max-w-4xl px-4 text-center sm:px-6">
            <p className="text-xs uppercase tracking-[0.2em] text-[#d4a843]">{t("cta.ready")}</p>
